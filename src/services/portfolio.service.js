...
const { portfolioToRow, rowToPortfolio, toBqDatetime } = require('./portfolio.mappers')
...
async function executeBuyTrade(portfolioId, symbol, quantity, price) {
  ...
  try {
    ...
    if (existingPosition) {
      ...
      // Lazy-backfill: older positions may have been written before the
      // asset_class field existed. Add it on the next buy rather than
      // waiting for the backfill script to touch every row.
      if (!existingPosition.asset_class) {
        existingPosition.asset_class = await classifyAsset(symbol)
      }
    } else {
      // Tag new positions with their asset class. classifyAsset returns
      // null on any failure — the read path and mobile both default
      // missing values to Stock, so a classifier outage can't block a
      // trade that has otherwise been authorized.
